    ...
    def parallel_crawler(self, movie: Movie, tqdm_bar=None) -> Dict[str, MovieInfo]:
        ...
        for mod_partial, info in all_info.items():
            ...
            if hasattr(sys.modules[mod], 'parse_data_raw'):
                ...
                th = threading.Thread(target=wrapper, name=mod, args=(parser, info, self.config.network.retry))
            th.start()
        ...
        # 根据抓取结果更新影片类型判定
        if movie.data_src == 'cid' and movie.dvdid:
            titles = [all_info[i].title for i in self.config.crawler.selection[movie.data_src]]
            if any(titles):
                movie.dvdid = None
                all_info = {k: v for k, v in all_info.items() if k in self.config.crawler.selection['cid']}
            else:
                logger.debug(f'自动更正影片数据源类型: {movie.dvdid} ({movie.cid}): normal')
                movie.data_src = 'normal'
                movie.cid = None
                all_info = {k: v for k, v in all_info.items() if k not in self.config.crawler.selection['cid']}
    ...
    def summarize_info(self, movie: Movie, all_info: Dict[str, MovieInfo]) -> bool:
        ...
        if self.config.summarizer.title.remove_trailing_actor_name:
            for name, data in all_info.items():
                data.title = self._remove_trail_actor_in_title(data.title, data.actress)
        ...
        if self.config.crawler.respect_site_avid:
            ...
            # 根据权重选择最终番号
            if id_weight:
                sorted_id_weight = {k: v for k, v in sorted(id_weight.items(), key=lambda x: len(x[1]), reverse=True)}
        ...
        if javdb_cover is not None:
            match self.config.crawler.use_javdb_cover:
                case 'fallback':
